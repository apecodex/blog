...
const saveCategoryHandle = async () => {
    if (newCategoryName.value.length !== 0) {
        ...
        form.append("name", newCategoryName.value)
        await saveOrUpdateCategory(form).then((resp: ResultObject<null>) => {
...
const deleteCategoryHandle = async () => {
    ...
    const form = new FormData()
    form.append("categoryId", deleteId.value)
    await deleteCategory(form).then((resp: ResultObject<null>) => {
            if (resp.status) {
                createCategoryBackData({current: pagination.page})
                tipMessage.type = 'success'
                tipMessage.content = resp.message
                ...
                return h(
                    NInput,
                    {
                        ...
                        async onChange(value: string | [string, string]) {
                            ...
                            const form = new FormData()
                            form.append("categoryId", row.id)
                            form.append("name", value as string)
                            await saveOrUpdateCategory(form).then((resp) => {
                                if (resp.status) {
