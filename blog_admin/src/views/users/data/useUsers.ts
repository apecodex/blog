            ...
            h(
              NButton,
              {
                ...
                onClick: async () => {
                  ...
                  await updateUserDisabledStatus({ id: row.userAuthId, enable: !row.enable }).then((resp: ResultObject<null>) => {
                    if (resp.status) {
...
const createUsersData = async (condition: ConditionParams) => {
  ...
  await listUserBack(condition).then((resp: PageResult<Array<UsersBackModel>>) => {
    if (resp.code === StatusCode.SUCCESS && resp.status) {
      usersData.push(...resp.data.recordList)
...
const sendNoticeHandle = async () => {
  ...
  await sendNotice({ userId: sendNoticeForm.userId, title: sendNoticeForm.title, content: sendNoticeForm.content }).then((resp: ResultObject<null>) => {
    if (resp.status) {
      tipMessage.type = "success";
      tipMessage.content = resp.message;
    } else {
