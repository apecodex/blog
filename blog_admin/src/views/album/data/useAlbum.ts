...
const updateAlbumHandle = async () => {
    if (checkAlbumInfo) {
        ...
        form.append('albumName', albumFormInfo.albumName.content);
        form.append('albumId', albumFormInfo.id);
        form.append('albumDesc', albumFormInfo.albumDesc);
        form.append('albumCover', albumFormInfo.albumCover.url);
        form.append('status', albumFormInfo.status as any);
        let tipMessage = (window as any).$message.create("正在修改...", {
            type: 'loading'
        });
        await updateAlbum(form).then((resp: ResultObject<null>) => {
            if (resp.code === StatusCode.SUCCESS && resp.status) {
...
const newAlbumHandle = async () => {
    if (checkAlbumInfo) {
        ...
        await saveAlbum(form).then((resp: ResultObject<null>) => {
            if (resp.code === StatusCode.SUCCESS && resp.status) {
                albumData.value.length = 0
