...
<template>
  <n-space vertical>
    <n-card>
      <n-tabs class="card-tabs" default-value="websiteInfo" size="large" animated style="margin: 0 -4px"
          ...
          <n-space vertical v-if="websiteData">
            ...
            <n-space vertical>
              ...
              <comment-comp :style="{ width: '350px' }" maxlength="150" show-count enter-btn-text="保存" :min-rows="3"
                :max-rows="3" v-model:default-value="websiteData.websiteIntro" @enter="updateWebsiteIntro" size="small" />
            </n-space>
            ...
            <n-space vertical>
              ...
              <n-text v-for="(value, key) in websiteData.socialLogin" :key="key">
                <n-space align="center">
                  ...
                  <n-switch :round="false" v-model:value="websiteData.socialLogin[key]" @update:value="updateSocialLogin">
                    <template #checked>
                      开启
                    </template>
                    <template #unchecked>
        ...
        <n-tab-pane name="websiteNotice" tab="网站公告">
          <n-space vertical v-if="websiteData">
            <comment-comp maxlength="200" show-count enter-btn-text="保存" :min-rows="4" :max-rows="4"
              v-model:default-value="websiteData.websiteNotice" @enter="updateWebsiteNotice" size="small" />
          </n-space>
        </n-tab-pane>
        <n-tab-pane name="chatRoomNotice" tab="聊天室公告">
          <n-space vertical v-if="websiteData">
            <comment-comp maxlength="200" show-count enter-btn-text="保存" :min-rows="4" :max-rows="4"
              v-model:default-value="websiteData.chatRoomNotice" @enter="updateChatRoomNoticeNotice" size="small" />
          </n-space>
        </n-tab-pane>
        <n-tab-pane name="websiteBackgroundImages" tab="首页背景图">
          <n-space vertical v-if="websiteData">
            <n-upload :default-file-list="websiteBackgroundImagesFile" list-type="image-card" :max="6" multiple
              @before-upload="beforeUpload" @remove="websiteBackgroundImagesRemoveHandle"
        ...
        <n-tab-pane name="websiteAvatar" tab="网站头像">
          <n-space align="center">
            <n-space vertical>
              ...
              <n-upload :default-file-list="websiteAvatarFile" list-type="image-card" :max="1"
                @before-upload="beforeUpload" :custom-request="updateWebsiteAvatarHandle">
                点击上传
              </n-upload>
            </n-space>
            <n-space vertical>
              <n-text type="warning">默认用户头像</n-text>
              <n-upload :default-file-list="websiteDefaultUserAvatarFile" list-type="image-card" :max="1"
                @before-upload="beforeUpload" :custom-request="updateWebsiteDefaultUserAvatarHandle">
                点击上传
              </n-upload>
            </n-space>
            <n-space vertical>
              <n-text type="warning">默认游客头像</n-text>
              <n-upload :default-file-list="websiteDefaultTouristAvatarFile" list-type="image-card" :max="1"
                @before-upload="beforeUpload" :custom-request="updateWebsiteDefaultTouristAvatarHandle">
                点击上传
              </n-upload>
            </n-space>
          </n-space>
        ...
        <n-tab-pane name="websiteQRCode" tab="打赏码">
          <n-space align="center">
            <n-space vertical>
              ...
              <n-upload :default-file-list="weiXinQRCodeFile" list-type="image-card" :max="1"
                @before-upload="beforeUpload" :custom-request="updateWeiXinQRCodeFileHandle">
                点击上传
              </n-upload>
            </n-space>
            <n-space vertical>
              <n-text type="warning">支付宝打赏码</n-text>
              <n-upload :default-file-list="alipayQRCodeFile" list-type="image-card" :max="1"
                @before-upload="beforeUpload" :custom-request="updateAlipayQRCodeFileHandle">
                点击上传
              </n-upload>
            </n-space>
          </n-space>
