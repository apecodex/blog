...
const saveOrUpdateResourceHandle = async () => {
  ...
  if (resourceForm.name.trim().length === 0) {
    tipMessage.type = 'warning'
    tipMessage.content = '资源名称不能为空'
    return;
  }
  const resourceParams: ResourceParams = {
    id: resourceForm.id ? resourceForm.id : '',
    isAnonymous: resourceForm.isAnonymous as any,
    isEnable: resourceForm.isEnable as any,
    name: resourceForm.name,
    parentId: resourceForm.parentId ? resourceForm.parentId : '',
    requestMethod: resourceForm.requestMethod ? resourceForm.requestMethod : '',
    url: resourceForm.url ? resourceForm.url : '',
  }
  await saveOrUpdateResource(resourceParams).then((resp: ResultObject<null>) => {
    if (resp.status) {
