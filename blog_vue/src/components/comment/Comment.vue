...
const saveComment = () => {
  if (!userInfoStore.isLogin && props.hasLogin) {
    notify({
      text: '请先登录',
      type: 'warn',
    });
...
<template>
  <div class="w-full relative" :class="{ 'overflow-hidden': !showCommentTools }">
    <div class="comment-no-login" :class="{ 'active': !userInfoStore.isLogin && hasLogin }">
    ...
    <textarea v-model="commentText"
      class="comment-textarea w-full h-60px bg-transparent text-16px outline-none resize-none" :placeholder="placeholder"
      @focusin="showCommentTools = true" @blur="onBlur" ref="commentTargetRef" />
    ...
    <div
      ...
      <div class="p-3px" style="border-top: 1px dashed var(--theme-bg-reverse)">
        <div class="h-full flex gap-12px">
          <img v-lazy class="w-30px h-30px" :data-src="smilingFaces['[微笑]']" :src="emojiLoadingImg" title="笑脸" alt="微笑"
            @click="emojiIndex = 1" />
          <img v-lazy class="w-30px h-30px" :data-src="humans['[耸肩]']" :src="emojiLoadingImg" title="人类和身体" alt="耸肩"
            @click="emojiIndex = 2" />
          <img v-lazy class="w-30px h-30px" :data-src="animalsAndNature['[独角兽]']" :src="emojiLoadingImg" title="自然和食物"
            alt="独角兽" @click="emojiIndex = 3" />
          <img v-lazy class="w-30px h-30px" :data-src="others['[火箭]']" :src="emojiLoadingImg" title="载具和活动" alt="火箭"
            @click="emojiIndex = 4" />
        </div>
      </div>
    </div>
