...
// 解析emoji
export const parseComment = (comment: string, width: string, height: string, customClass?: string) => {
  let reg = /\[.+?\]/g;
  return comment.replace(reg, (str: string) => {
    return `<img class="inline-block mx-1px align-text-bottom ${customClass ? customClass : ''}" title="${str}" src="${emojis[str]}" width="${width}" height="${height}" alt="" />`;
  })
}
